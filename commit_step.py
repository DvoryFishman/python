...
def commit_func(path, name_commit):
    ...
    current_id = os.path.join(commit_path, "current_id.txt")
    ...
    if num > 1 and bool_compare_folder(
        os.path.join(wit_path, "add"), os.path.join(in_prev_commit, "files")
    ):
    ...
    result = copy_to_folder(add_path, os.path.join(name_folder, "files"))
    ...
    write_number_to_file(current_id, str(num))
